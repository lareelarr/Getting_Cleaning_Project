...
# select only columns corresponding to Subject, Activity, mean() and std()
temp2 <- select(temp1, Subject, Activity, contains("mean()"),contains("std()"))
...
temp4 <- select(temp3, Subject, ActivityName, contains("mean()"),contains("std()"))
temp5 <- temp4
...
ActivityGroup <- group_by(tidyDF1,ActivityName, Subject)
